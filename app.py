"""Interactive dashboard for the Steam games dataset in stem games mk 2.zip."""
...
USE_COLS = [
    ...
    "Price", "DiscountDLC count", "Supported languages", "Reviews", "Windows", "Mac",
    "Linux", "Metacritic score", "Positive", "Negative", "Recommendations",
    "Average playtime forever", "Average playtime two weeks", "Developers", "Publishers",
...
@st.cache_data(show_spinner="Reading the games dataset from the ZIP file…")
def load_games(zip_path: str, modified: float) -> pd.DataFrame:
    """Read just the analysis columns, directly from the compressed archive."""
    ...
    for col in ["Peak CCU", "Positive", "Negative", "Recommendations", "Metacritic score",
                "Average playtime forever", "Average playtime two weeks", "Required age"]:
...
with st.sidebar:
    st.header("Dataset")
    zip_text = st.text_input("Path to dataset ZIP", value=str(DEFAULT_ZIP))
    uploaded = st.file_uploader("Or select the ZIP file", type="zip")
    ...
    st.caption("The archive should contain games.csv. The large JSON copy is not needed.")

zip_path = Path(zip_text).expanduser()
if not zip_path.is_file():
    st.info("Enter the location of **stem games mk 2.zip** in the sidebar to load the dashboard.")
    st.code(r"C:\Users\Yash\OneDrive\Desktop\BCA\Sem 3\Descriptive Analytics\stem games mk 2.zip")
    st.stop()

try:
    games = load_games(str(zip_path.resolve()), zip_path.stat().st_mtime)
except Exception as exc:
    st.error(f"Could not read this archive: {exc}")
    st.stop()
